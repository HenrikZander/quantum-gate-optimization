...
def getCircuitVariables():
    ...
    data["frequencies"] = [frequencyQ1.get(), frequencyQ2.get(),
                           frequencyCoupler.get()]
    data["anharmonicities"] = [anharmonicityQ1.get(), anharmonicityQ2.get(),
                               anharmonicityCoupler.get()]
...
def selectSaveFolder():
    folderPath = filedialog.askdirectory(
        title="Choose folder to save results in", initialdir="C:/")
    if folderPath:
...
def loadCircuit():
    circuitFile = filedialog.askopenfilename(
        title="Select circuit", defaultextension='.json', filetypes=[("JSON files (.json)", '*.json')])
    if circuitFile:
...
def exportCircuit():
    ...
    path = filedialog.asksaveasfilename(title="Save circuit", defaultextension='.json', filetypes=[
...
def optimizeStatusFrame(bottomFrame):
    ...
    startOptimizeButton = Button(bottomFrame, text="Start Optimizing",
                                 command=startOptimizing, padx=3, pady=3, background="green")
    startOptimizeButton.grid(column=2, row=0)

    global stopOptimizeButton
    stopOptimizeButton = Button(
        bottomFrame, text="Stop Optimizing", padx=3, pady=3, background="grey")
    stopOptimizeButton.grid(column=2, row=1)

    progressFrame = Frame(bottomFrame, height=(
        1-relativeHeight)*height, width=width*0.80)
    progressFrame.grid(column=0, row=0, rowspan=2, columnspan=2)
    progressFrame.grid_propagate(0)

    progressLabel = Label(bottomFrame, font=(
        "Helvetica", 11), textvariable=statusString)
...
def generateSaveFolderInput(circuitFrame, entryCharacterWidth):
    ...
    saveFolderTitleFrame = Frame(
        circuitFrame, height=50, width=relativeWidth*width*0.40)  # , background="red")
    saveFolderTitleFrame.grid(row=6, column=0, columnspan=3)

    saveFolderTitle = Label(
        saveFolderTitleFrame, text='Configure result output', font=('Helvetica', 12))
    ...
    saveFolderFrameOuter = Frame(
        circuitFrame, height=50, width=relativeWidth*width*0.40)  # , background="orange")
    ...
    saveFolderEntry = Entry(saveFolderFrameInner, state="readonly",
                            width=45, readonlybackground="white", textvariable=resultFolderPath)
    saveFolderEntry.pack(side=LEFT)

    saveFolderButton = Button(saveFolderFrameInner, text="Select Folder",
                              command=selectSaveFolder, background="#21e4d7")
    saveFolderButton.pack(side=LEFT)

    numOfSessionsFrameOuter = Frame(
        circuitFrame, height=50, width=relativeWidth*width*0.40)  # , background="red")
    ...
    numOfSessionsLabel = Label(
        numOfSessionsFrameInner, text="Number of consecutive optimizations to run: ")
    numOfSessionsLabel.pack(side=LEFT)

    numOfSessionsInputBox = ttk.Spinbox(numOfSessionsFrameInner, from_=1, to=10,
                                        state="readonly", width=4, textvariable=numberOfConsecutiveSessions)
...
def generateCouplingInputs(circuitFrame, entryCharacterWidth):
    couplingInputFrameOuter = Frame(
        circuitFrame, height=60, width=relativeWidth*width*0.40)  # , background="orange")
    couplingInputFrameOuter.grid(row=3, column=0, columnspan=3)

    couplingInputFrameInner = LabelFrame(
        couplingInputFrameOuter, text="Coupling strength to coupler [GHz]: ")
    ...
    couplingEntryQ1 = Entry(couplingInputFrameInner,
                            width=entryCharacterWidth, textvariable=couplingQ1)
    ...
    couplingEntryQ2 = Entry(couplingInputFrameInner,
                            width=entryCharacterWidth, textvariable=couplingQ2)
    couplingEntryQ2.pack(side=LEFT, padx=(0, 5))


def generateAnharmonicityInputs(circuitFrame, entryCharacterWidth):
    anharmonicityInputFrameOuter = Frame(
        circuitFrame, height=60, width=relativeWidth*width*0.40)  # , background="orange")
    anharmonicityInputFrameOuter.grid(row=2, column=0, columnspan=3)

    anharmonicityInputFrameInner = LabelFrame(
        anharmonicityInputFrameOuter, text="Anharmonicites [GHz]: ")
    ...
    anharmonicityEntryQ1 = Entry(
        anharmonicityInputFrameInner, width=entryCharacterWidth, textvariable=anharmonicityQ1)
    ...
    anharmonicityEntryQ2 = Entry(
        anharmonicityInputFrameInner, width=entryCharacterWidth, textvariable=anharmonicityQ2)
    anharmonicityEntryQ2.pack(side=LEFT)

    anharmonicityLabelCoupler = Label(
        anharmonicityInputFrameInner, text="Coupler: ")
    anharmonicityLabelCoupler.pack(side=LEFT, padx=(10, 0), pady=5)

    anharmonicityEntryCoupler = Entry(
        anharmonicityInputFrameInner, width=entryCharacterWidth, textvariable=anharmonicityCoupler)
    anharmonicityEntryCoupler.pack(side=LEFT, padx=(0, 5))


def generateFrequencyInputs(circuitFrame, entryCharacterWidth):
    frequencyInputFrameOuter = Frame(
        circuitFrame, height=60, width=relativeWidth*width*0.40)  # , background="orange")
    frequencyInputFrameOuter.grid(row=1, column=0, columnspan=3)

    frequencyInputFrameInner = LabelFrame(
        frequencyInputFrameOuter, text="Frequencies [GHz]: ")
    ...
    frequencyEntryQ1 = Entry(frequencyInputFrameInner,
                             width=entryCharacterWidth, textvariable=frequencyQ1)
    ...
    frequencyEntryQ2 = Entry(frequencyInputFrameInner,
                             width=entryCharacterWidth, textvariable=frequencyQ2)
    ...
    frequencyEntryCoupler = Entry(
        frequencyInputFrameInner, width=entryCharacterWidth, textvariable=frequencyCoupler)
    frequencyEntryCoupler.pack(side=LEFT, padx=(0, 5))


def generateCircuitInputControls(circuitFrame):
    controlsInputFrameOuter = Frame(
        circuitFrame, height=80, width=relativeWidth*width*0.40)  # , background="orange")
    ...
    loadCircuitButton = Button(controlsInputFrameInner, text="Load Circuit",
                               command=loadCircuit, padx=3, pady=3, background="#21e4d7")
    loadCircuitButton.grid(row=0, column=0, padx=4, pady=4)

    changeDefaultCircuitButton = Button(controlsInputFrameInner, text="Set to Default",
                                        command=changeDefaultCircuit, padx=3, pady=3, background="#21e4d7")
    changeDefaultCircuitButton.grid(row=0, column=1, padx=4, pady=4)

    useDefaultCircuitButton = Button(controlsInputFrameInner, text="Use Default",
                                     command=useDefaultCircuit, padx=3, pady=3, background="#21e4d7")
    useDefaultCircuitButton.grid(row=0, column=2, padx=4, pady=4)

    generateCircuitButton = Button(controlsInputFrameInner, text="Export",
                                   command=exportCircuit, padx=3, pady=3, background="#21e4d7")
...
def generateCircuitInputs(circuitFrame):
    ...
    circuitTitleFrame = Frame(
        circuitFrame, height=50, width=relativeWidth*width*0.40)  # , background="red")
    circuitTitleFrame.grid(row=0, column=0, columnspan=3)

    circuitTitle = Label(
        circuitTitleFrame, text='Circuit parameters', font=('Helvetica', 12))
...
def optimizeCircuitParameterFrame(topFrame):
    ...
    circuitFrame = Frame(topFrame, height=relativeHeight *
                         height, width=relativeWidth*width*0.40)
...
def generateModulationTimeBoundaryInput(inputBoundaryFrame, entryCharacterWidth):
    modulationTimeInputFrameOuter = Frame(
        inputBoundaryFrame, height=35, width=relativeWidth*width*0.60)  # , background="green")
    modulationTimeInputFrameOuter.grid(row=4, column=0, columnspan=3)

    modulationTimeInputFrameInner = Frame(
        modulationTimeInputFrameOuter)  # , background="blue")
    modulationTimeInputFrameInner.place(anchor="e", relx=0.8, rely=0.5)

    modulationTimeLabel = Label(
        modulationTimeInputFrameInner, text="Total modulation time of AC-flux [ns]:")
    modulationTimeLabel.pack(side=LEFT, padx=(0, 5))

    lowerModulationTimeLabel = Label(
        modulationTimeInputFrameInner, text="Lower limit:")
    lowerModulationTimeLabel.pack(side=LEFT)

    lowerModulationTimeEntry = Entry(
        modulationTimeInputFrameInner, width=entryCharacterWidth, textvariable=modulationTimeLower)
    lowerModulationTimeEntry.pack(side=LEFT, padx=(0, 5))

    upperModulationTimeLabel = Label(
        modulationTimeInputFrameInner, text="Upper limit:")
    upperModulationTimeLabel.pack(side=LEFT)

    upperModulationTimeEntry = Entry(
        modulationTimeInputFrameInner, width=entryCharacterWidth, textvariable=modulationTimeUpper)
    upperModulationTimeEntry.pack(side=LEFT)


def generateOmegaPhiBoundaryInput(inputBoundaryFrame, entryCharacterWidth):
    omegaPhiInputFrameOuter = Frame(
        inputBoundaryFrame, height=35, width=relativeWidth*width*0.60)  # , background="yellow")
    omegaPhiInputFrameOuter.grid(row=3, column=0, columnspan=3)

    omegaPhiInputFrameInner = Frame(
        omegaPhiInputFrameOuter)  # , background="blue")
    omegaPhiInputFrameInner.place(anchor="e", relx=0.8, rely=0.5)

    omegaPhiLabel = Label(omegaPhiInputFrameInner,
                          text="Frequency \u03C9 of AC-flux [GHz]:")
    ...
    lowerOmegaPhiEntry = Entry(
        omegaPhiInputFrameInner, width=entryCharacterWidth, textvariable=omegaPhiLower)
    ...
    upperOmegaPhiEntry = Entry(
        omegaPhiInputFrameInner, width=entryCharacterWidth, textvariable=omegaPhiUpper)
    upperOmegaPhiEntry.pack(side=LEFT)


def generateDeltaBoundaryInput(inputBoundaryFrame, entryCharacterWidth):
    deltaInputFrameOuter = Frame(
        inputBoundaryFrame, height=35, width=relativeWidth*width*0.60)  # , background="green")
    ...
    deltaLabel = Label(deltaInputFrameInner,
                       text="Amplitude of \u03B4(t) [\u03A6"+subscriptZero+"]:")
    ...
    lowerDeltaEntry = Entry(deltaInputFrameInner,
                            width=entryCharacterWidth, textvariable=deltaLower)
    ...
    upperDeltaEntry = Entry(deltaInputFrameInner,
                            width=entryCharacterWidth, textvariable=deltaUpper)
    upperDeltaEntry.pack(side=LEFT)


def generateThetaBoundaryInput(inputBoundaryFrame, entryCharacterWidth):
    thetaInputFrameOuter = Frame(
        inputBoundaryFrame, height=35, width=relativeWidth*width*0.60)  # , background="yellow")
    ...
    thetaLabel = Label(thetaInputFrameInner,
                       text="Strength of DC-flux \u0398 [\u03A6"+subscriptZero+"]:")
    ...
    lowerThetaEntry = Entry(thetaInputFrameInner,
                            width=entryCharacterWidth, textvariable=thetaLower)
    ...
    upperThetaEntry = Entry(thetaInputFrameInner,
                            width=entryCharacterWidth, textvariable=thetaUpper)
...
def generateBoundaryInput(inputBoundaryFrame):
    ...
    riseTimeFrameOuter = Frame(inputBoundaryFrame, height=35,
                               width=relativeWidth*width*0.60)  # , background="red")
    ...
    riseTimeLabel = Label(
        riseTimeFrame, text="Rise time of \u03B4(t) (from 0% to 100%) [ns]:")
    riseTimeLabel.pack(side=LEFT, padx=(0, 5))

    riseTimeEntry = ttk.Spinbox(
        riseTimeFrame, from_=15, to=100, textvariable=riseTime, width=4, state="readonly")
    ...
    generateModulationTimeBoundaryInput(
        inputBoundaryFrame, entryCharacterWidth)


def generateSelectEnergyLevels(settingsFrameRight):
    selectEnergyLevelsFrameOuter = Frame(
        settingsFrameRight, height=55, width=relativeWidth*width*0.30)  # , background="blue")
    selectEnergyLevelsFrameOuter.grid(row=1, column=0)

    selectEnergyLevelsFrameInner = Frame(
        selectEnergyLevelsFrameOuter)  # , background="orange")
    selectEnergyLevelsFrameInner.place(anchor="n", relx=0.5, rely=0.1)

    selectEnergyLevelsTitle = Label(
        selectEnergyLevelsFrameInner, text="Number of energy levels per qubit:")
    selectEnergyLevelsTitle.pack(side=LEFT, padx=(0, 5))

    selectEnergyLevels = ttk.Spinbox(
        selectEnergyLevelsFrameInner, from_=2, to=7, textvariable=energyLevels, width=4, state="readonly")
    selectEnergyLevels.set(3)
    selectEnergyLevels.pack(side=LEFT)


def generateSelectGate(settingsFrameRight):
    selectGateFrameOuter = Frame(
        settingsFrameRight, height=55, width=relativeWidth*width*0.30)  # , background="orange")
    selectGateFrameOuter.grid(row=0, column=0)
    selectGateFrameOuter.pack_propagate(0)

    selectGateFrameInner = LabelFrame(
        selectGateFrameOuter, text="Gate to optimize for: ")
    selectGateFrameInner.place(anchor='n', relx=0.5, rely=0)

    selectISWAP = Radiobutton(
        selectGateFrameInner, text="iSWAP", value="iSWAP", variable=selectedGate)
    selectISWAP.select()
    selectISWAP.pack(side=LEFT)

    selectSWAP = Radiobutton(selectGateFrameInner,
                             text="SWAP", value="SWAP", variable=selectedGate)
    selectSWAP.pack(side=LEFT)

    selectCZ = Radiobutton(selectGateFrameInner, text="CZ",
                           value="CZ", variable=selectedGate)
    selectCZ.pack(side=LEFT)


def generateSelectAlgorithm(settingsFrameLeft):
    selectAlgorithmFrameOuter = Frame(
        settingsFrameLeft, height=110, width=relativeWidth*width*0.30)  # , background="orange")
    selectAlgorithmFrameOuter.grid(row=0, column=0)

    selectAlgorithmFrameInner = LabelFrame(
        selectAlgorithmFrameOuter, text="Algorithms to use: ")
    selectAlgorithmFrameInner.place(anchor='n', relx=0.5, rely=0)

    selectDifferentialEvolutionBox = Checkbutton(
        selectAlgorithmFrameInner, text="Differential Evolution (recommended)", variable=runDifferentialEvolution)
    selectDifferentialEvolutionBox.select()
    selectDifferentialEvolutionBox.grid(row=0, column=0, sticky='w')

    selectSimplicalHomologyBox = Checkbutton(
        selectAlgorithmFrameInner, text="Simplical Homology Global", variable=runSimplicalHomologyGlobal)
    selectSimplicalHomologyBox.grid(row=1, column=0, sticky='w')

    selectDualAnnelingBox = Checkbutton(
        selectAlgorithmFrameInner, text="Dual Anneling", variable=runDualAnneling)
    selectDualAnnelingBox.grid(row=2, column=0, sticky='w')


def generateBoundarySettings(settingsBoundaryFrame):
    selectSignalFrameOuter = Frame(
        settingsBoundaryFrame, height=35, width=relativeWidth*width*0.60)  # , background="blue")
    ...
    selectSignalTitle = Label(selectSignalFrameInner,
                              text="Shape of magnetic flux signal (\u03A6):")
    selectSignalTitle.pack(side=LEFT, padx=(0, 5))

    selectSignal = ttk.Combobox(selectSignalFrameInner, state="readonly", values=(
        '\u03A6 = \u0398 + \u03B4(t) \u2022 cos(\u03C9t)', 'Arccos-signal (not implemented)'), textvariable=signalShape)
    selectSignal.current(0)
    selectSignal.pack(side=LEFT)

    # , highlightbackground="black", highlightthickness=1)#, background="blue")
    inputBoundaryFrame = Frame(
        settingsBoundaryFrame, height=175, width=relativeWidth*width*0.60)
...
def optimizerSettingsFrame(topFrame):
    ...
    settingsFrame = Frame(topFrame, height=relativeHeight*height,
                          width=relativeWidth*width*0.60)  # , background="yellow")
    settingsFrame.grid(row=0, column=2)
    settingsFrame.grid_propagate(0)

    settingsTitleFrame = Frame(settingsFrame, height=settingsTitleFrameHeight,
                               width=relativeWidth*width*0.60)  # , background="orange")
    settingsTitleFrame.grid(row=0, column=0, columnspan=2)

    settingsTitle = Label(
        settingsTitleFrame, text='Optimizer configuration', font=('Helvetica', 12))
    ...
    settingsFrameLeft = Frame(settingsFrame, height=optimizeSettingsFrameHeight,
                              width=relativeWidth*width*0.30)  # , background="green")
    settingsFrameLeft.grid(column=0, row=1)
    settingsFrameLeft.grid_propagate(0)

    settingsFrameRight = Frame(settingsFrame, height=optimizeSettingsFrameHeight,
                               width=relativeWidth*width*0.30)  # , background="red")
    ...
    settingsBoundaryTitleFrame = Frame(
        settingsFrame, height=settingsTitleFrameHeight, width=relativeWidth*width*0.60)  # , background="orange")
    settingsBoundaryTitleFrame.grid(row=3, column=0, columnspan=2)

    settingsBoundaryTitle = Label(
        settingsBoundaryTitleFrame, text='Magnetic flux signal and search boundaries', font=('Helvetica', 12))
    settingsBoundaryTitle.configure(font=titleSettingsFont)
    settingsBoundaryTitle.place(anchor='center', relx=0.5, rely=0.5)

    settingsBoundaryFrame = Frame(settingsFrame, height=relativeHeight*height-optimizeSettingsFrameHeight -
                                  2*settingsTitleFrameHeight, width=relativeWidth*width*0.60)  # , background="orange")
